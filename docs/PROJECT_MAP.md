...
| `src/app/(app)/dashboard/page.tsx` | Progress dashboard: streak, words learned, grammar, 12-week activity calendar, minutes per strand, fluency WPM (route `/dashboard`) |
| `src/app/manifest.ts`, `src/app/apple-icon.png`, `public/icon-*.png` | PWA manifest + icons |
| `public/sw.js`, `public/offline.html` | Service worker: cache-first build assets, network-first pages with last-good-copy fallback, offline page. Registered by `components/ServiceWorker.tsx` (production only) |
...
| `src/lib/dates.ts` | Local-day math + streaks (4 core steps = a completed day; weekends without a session don't break a streak) — pure |
| `src/lib/day-server.ts` | Learner's timezone (from the `tz` cookie set by `TimezoneCookie`), today's local date, `recordStep` → `record_step()` RPC |
| `src/lib/overview.ts` | Read models for «Сегодня» (steps done, streak) and the dashboard |
...
| `supabase/migrations/0009_…` | Фаза 4: `sessions.completed_steps` + `grammar_minutes`, `record_step(date, step, minutes)` |
...
| `/` | required | "Сегодня" — streak, 4 daily steps with done marks, next step highlighted |
...
| `/dashboard` | required | Progress stats |
...
- ✅ **Фаза 4** — PWA (manifest, icons, service worker, offline page), `experimental.useOffline` (Server Actions retry after reconnect; vocab reviews queue while offline), step tracking per local day, streaks, «Дальше →» between steps, real dashboard.
- ⬜ **Фаза 5** (optional) — paid TTS/STT, placement test, AI writing feedback.
